...
;
(function ($, Drupal, drupalSettings) {
    Drupal.behaviors.jobsAjaxCall = {
        attach: function () {
            var initial_page_count = drupalSettings.items_per_block;
            var per_page_count = drupalSettings.per_page_count;
            var job_board = drupalSettings.board_token;
            var job_url = drupalSettings.board_url;
            var html_region_filters_to_append = '';
            var html_location_filters_to_append = '';
            var html_department_filters_to_append = '';
            var html_company_filters_to_append = '';
            var regions_new = [];
            var location_new = [];
            var department_new = {};
            var company_new = [];
            ...
            var locations = [];
            var dependentFilter = [];
            var dependentValue = JSON.parse(localStorage.getItem('Region'));
            var jobsAPI = job_url + job_board + '/jobs?content=true';
            $.getJSON( jobsAPI, {
              format: "json"
            }).done(function(jobsdata) {
            if (jobsdata.length == 0) {
                $("ul.gh-jobs-list").html("<div><div class= 'no-results-wrp'><h2>No results found</h2><p class='text-center'> Sorry, there are currently no jobs matching your search criteria.<br>We encourage you to create a profile so that a Recruiter can be in touch about relevant openings in the future.<br>Please also be sure to check back for our most recent openings. </p></div></div>");
                $(".more-oppotunities").hide();
            }
            else {
            jobsFilterData = jobsdata.jobs;
            parseData(jobsFilterData, initial_page_count);
            }
            function filters(jobsFilterData , dependentValue) {
                location_new = [];
                locations = [];
                department_new = {};
                company_new = [];
                $.each(jobsFilterData, function (i, datajob) {
                    var dept=datajob.departments[0];
                    if (typeof(dept) !== 'undefined') {
                        department_new[dept.id] = dept.name;
                    }
                    $.each(datajob.metadata, function (i, values){
                    if (values.name == 'Company') {
                        if(!company_new.includes(values.value)) {
                          company_new.push(values.value);
                        }
                    }
                    else if (values.name == 'Region') {
                        if(!regions_new.includes(values.value)) {
                          regions_new.push(values.value);
                        }
                        dependentFilter.push({key:values.value, value:datajob.location.name});
                    }
                    if(dependentValue == 'All'){
                        str_arr = datajob.location.name.split(',');
                        if ($.inArray(datajob.location.name, result)){
                            location_new.push(datajob.location.name);
                            if(!sort_country.includes(str_arr[2])) {
                            sort_country.push(str_arr[2]);
                            }
                            result.push(datajob.location.name);
                        }
                    }
                    else{
                        dependentFilter = dependentFilter.filter((data) => data.key === dependentValue);
                        const uniqueBy = (arr, prop) => {
                            return [...new Map(arr.map((m) => [m[prop], m])).values()];
                          };
                          var filterLocation =  uniqueBy(dependentFilter, 'value');
                          $.each(filterLocation, function (i, values){
                            str_arr = values.value.split(',');
                            if ($.inArray(values.value, result)){
                                location_new.push(values.value);
                                if(!sort_country.includes(str_arr[2])) {
                                sort_country.push(str_arr[2]);
                                }
                ...
                sort_country.sort();
                for(i=0;i<sort_country.length;i++) {
                   var key = sort_country[i];
                    for(j=0;j<location_new.length;j++) {
                       var country = location_new[j].split(',');
                       var value = country[2];
                        if(value == key){
                          if(!locations.includes(location_new[j])) {
                            locations.push(location_new[j]);
                          }
                        }
                    }
                }
                ...
                var sortedDepartments=sortProperties(department_new);
                regions_new.sort();
                html_region_filters_to_append = '<li data-value="All" class="select-dropdown__list-item1">All</li>';
                $.each(regions_new,function (i, region){
                    html_region_filters_to_append += '<li data-value="'+ region + '" class="select-dropdown__list-item1 region">' + region + '</li>';
                    $("ul.select-dropdown__list1").html(html_region_filters_to_append);
                });
                html_location_filters_to_append = '<li data-value="All" class="select-dropdown__list-item">All</li>';
                $.each(locations,function (i, location){
                    html_location_filters_to_append += '<li data-value="'+ location + '" class="select-dropdown__list-item">'+ location + '</li>';
                    $("ul.select-dropdown__list").html(html_location_filters_to_append);
                });
                html_department_filters_to_append = '<li data-value="All" class="select-dropdown__list-item2">All</li>';
                $.each(sortedDepartments,function (deptkey, department){
                    html_department_filters_to_append += '<li data-value='+ department[0] + ' class="select-dropdown__list-item2">' + department[1]+ '</li>';
                    $("ul.select-dropdown__list2").html(html_department_filters_to_append);
                });
                html_company_filters_to_append = '<li data-value="All" class="select-dropdown__list-item3">All</li>';
                company_new.sort();
                $.each(company_new,function (i, company){
                    html_company_filters_to_append += '<li data-value="'+ company + '" class="select-dropdown__list-item3">' + company + '</li>';
                    $("ul.select-dropdown__list3").html(html_company_filters_to_append);
                });
            }
            $(document).ready(function() {
                var locName = JSON.parse(localStorage.getItem('Location'));
                var deptId =  JSON.parse(localStorage.getItem('DepartmentID'));
                var deptName =  JSON.parse(localStorage.getItem('Department'));
                var cmpName = JSON.parse(localStorage.getItem('Company'));
                var RegionName = JSON.parse(localStorage.getItem('Region'));
                if (locName == null || locName == 'All' ) {
                    $('.select-dropdown__button span').text('All');
                }
                else {
                    $('.select-dropdown__button span').text(locName);
                    $('.select-dropdown__button').attr('data-value',locName);
                }
                if ( RegionName == null || RegionName == 'All') {
                    $('.select-dropdown__button1 span').text('All');
                }
                else {
                    $('.select-dropdown__button1 span').text(RegionName);
                    $('.select-dropdown__button1').attr('data-value',RegionName);
                }
                if (deptName == null || deptName == 'All') {
                    $('.select-dropdown__button2 span').text('All');
                }
                else {
                    $('.select-dropdown__button2 span').text(deptName);
                    $('.select-dropdown__button2').attr('data-value',deptId);
                }
                if (cmpName == null || cmpName == 'All') {
                    $('.select-dropdown__button3 span').text('All');
                }
                else {
                    $('.select-dropdown__button3 span').text(cmpName);
                    $('.select-dropdown__button3').attr('data-value',cmpName);
                }
                var locName = (locName == null) ? 'All' : JSON.parse(localStorage.getItem('Location'));
                var deptId =  (deptId == null) ? 'All' : JSON.parse(localStorage.getItem('DepartmentID'));
                var deptName = (deptName == null) ? 'All' : JSON.parse(localStorage.getItem('Department'));
                var cmpName = (cmpName == null) ? 'All' : JSON.parse(localStorage.getItem('Company'));
                var RegionName = (RegionName == null) ? 'All' : JSON.parse(localStorage.getItem('Region'));
                jobsFilterData = jobsdata.jobs;
                if (locName != 'All' || deptId != 'All' || cmpName != 'All' || RegionName != 'All') {
                    $("#clear-filters").show();
                }
                else {
                    $("#clear-filters").hide();
                }
                filters(jobsFilterData , dependentValue);
                filtersData(locName, deptId, deptName, cmpName, RegionName);
            });
            function filtersData(locName, deptId, deptName, cmpName, RegionName) {
                if (RegionName != 'All') {
                    localStorage.setItem('Region', JSON.stringify(RegionName));
                    jobsFilterData = jobsFilterData.filter((data) => data.metadata.some((metadata) => metadata.value === RegionName));
                }
                else {
                    localStorage.setItem('Region', JSON.stringify(RegionName));
                }
                if (locName != 'All') {
                    localStorage.setItem('Location', JSON.stringify(locName));
                    jobsFilterData = jobsFilterData.filter(function (data, i) {
                        return data.location.name === locName;
                    });
                }
                else {
                    localStorage.setItem('Location', JSON.stringify(locName));
                ...
                if (cmpName != 'All') {
                    localStorage.setItem('Company', JSON.stringify(cmpName));
                    jobsFilterData = jobsFilterData.filter((data) => data.metadata.some((metadata) => metadata.name === "Company" && metadata.value === cmpName));
                }
                else {
                    localStorage.setItem('Company', JSON.stringify(cmpName));
                }
                if (deptId != 'All') {
                    localStorage.setItem('DepartmentID', JSON.stringify(deptId));
                    var deptName = $('.select-dropdown__button2 span').text();
                    localStorage.setItem('Department', JSON.stringify(deptName));
                    jobsFilterData = jobsFilterData.filter(function (data, i) {
                        if(typeof(data.departments[0]) !== 'undefined') {
                         return data.departments[0].id == deptId;
                        }
                    });
                }
                else {
            ...
            $(document).on("click", ".select-dropdown__list-item, .select-dropdown__list-item2, .select-dropdown__list-item3, .select-dropdown__list-item1", function () {
                var locName = $('.select-dropdown__button').attr('data-value');
                var deptId = $('.select-dropdown__button2').attr('data-value');
                var cmpName = $('.select-dropdown__button3').attr('data-value');
                var RegionName = $('.select-dropdown__button1').attr('data-value');
                var deptName = (deptName == null) ? '' : JSON.parse(localStorage.getItem('Department'));
                jobsFilterData = jobsdata.jobs;
                if (locName != 'All' || deptId != 'All' || cmpName != 'All' || RegionName != 'All') {
                    $("#clear-filters").show();
                }
                ...
                filters(jobsFilterData , RegionName);
                filtersData(locName, deptId, deptName, cmpName, RegionName);
            });
            $('.clear-filters').on('click', function () {
                $('.select-dropdown__button').attr('data-value', 'All');
                $('.select-dropdown__button2').attr('data-value', 'All');
                $('.select-dropdown__button3').attr('data-value', 'All');
                $('.select-dropdown__button1').attr('data-value', 'All');
                $('.select-dropdown__button span').text('All');
                $('.select-dropdown__button2 span').text('All');
                $('.select-dropdown__button3 span').text('All');
                $('.select-dropdown__button1 span').text('All');
                ...
                parseData(jobsdata.jobs, initial_page_count);
            });
            $('.more-careerslist a').click(function () {
                totalRecords = parseInt($('.border-tan').length) + parseInt(per_page_count);
                parseData(jobsFilterData, totalRecords);
            });
            function parseData(jobsFilterData, totalRecords) {
                jobsFilterData.sort(function(a, b){
                    var a1= a.updated_at;
                    var b1= b.updated_at;
                    return new Date(b1) - new Date(a1);
                });
                html_to_append = '';
                if (jobsFilterData.length == 0) {
                    html_to_append = "<div><div class= 'no-results-wrp'><h2>No results found</h2><p class='text-center'> Sorry, there are currently no jobs matching your search criteria.<br>We encourage you to create a profile so that a Recruiter can be in touch about relevant openings in the future.<br>Please also be sure to check back for our most recent openings. </p></div></div>";
                    $(".more-oppotunities").hide();
                } else {
                    if (jobsFilterData.length > totalRecords) {
                        $(".more-careerslist").show();
                    } else {
                        $(".more-careerslist").hide();
                    }
                    if (jobsFilterData.length !== 0) {
                        $(".more-careers").show();
                    } else {
                    ...
                    $.each(jobsFilterData, function (i, datajob) {
                        if (i < totalRecords) {
                            html_to_append += rederJobData(datajob);
                        }
                    });
                }
                $("ul.gh-jobs-list").html(html_to_append);
            }
            function rederJobData(item) {
                var companyName = '--';
                item.metadata.filter(function (metadata, key) {
                    if(metadata.name === "Company"){
                        companyName = metadata.value != null ? metadata.value : "--"
                    }
                });
                return '<li class="border-tan"><a id="jobLink" href="careers/' + item.id + '"<div class="items-start displyflex"><div class="w-full displyflex"><p class="lg50 careers--listings-title">' +
                    item.title +
                    '</p><div class="items-center lg25 careers--listings-loc">' +
                    companyName  +
                    '</div><div class="items-center lg25 careers--listings-loc">' +
                    item.location.name +
                    '</div></div><div class="none-sp-more">→</div><div class="none-desk-more">More <span>→</span> </div></div></a></li>';
            }
          });
        }
    }
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.gh_jobs_apply = {
    attach: function (context, settings) {
      var job_board = settings.GHJobs.board_token;
      var job_url = settings.GHJobs.board_url;
      const id = settings.GHJobs.jid;
      var jobDetailsAPI = job_url + job_board + '/jobs/' + id + '?pay_transparency=true';
      $.getJSON(jobDetailsAPI, {
        format: "json"
      }).done(function (jobdetails) {
        var companyName = '--';
        jobdetails.metadata.filter(function (metadata, key) {
          if (metadata.name === "Company") {
            companyName = metadata.value != null ? metadata.value : "--"
          }
        });
        var postDate = new Date(jobdetails.updated_at);
        ...
        var htmlDoc = parser.parseFromString(jobdetails.content, 'text/html');
        var content = htmlDoc.all[0].textContent;
        $('title').text(jobdetails.title + '|Ogilvy');
        $("div.careers--main-title").html('<h2>' + jobdetails.title + '</h2>');
        $("span.c-name").html(companyName);
        $("span.l-name").html(jobdetails.location.name);
        $("span.j-id").html('Job Posted Date: ' + formattedDate);
        $("div.careers--details-main").html(content + '<div id="grnhse_app">Loading...</div>');
        load(id);
      });
    }
  };
  ...
  function updateURLWithGhSrcParam(url) {
    const ghSrcParam = getQueryParameter("gh_src"); // Get 'gh_src' parameter from the current URL
    if (ghSrcParam) {
      if (url.includes("?")) {
        url += `&gh_src=${ghSrcParam}`; // Append 'gh_src' as a query parameter
      } else {
        url += `?gh_src=${ghSrcParam}`; // Add 'gh_src' as the first query parameter
      }
    }
    return url;
  }
  $(document).on("click", "#jobLink, #details-back", function (event) {
    event.preventDefault();
    const jobUrl = $(this).attr("href"); // Get the job URL from the link
    const updatedUrl = updateURLWithGhSrcParam(jobUrl);
